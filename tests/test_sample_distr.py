...
from sample_distr import UniVarSampleDistr

DEFAULT_OBSERVATIONS = [3, 1, 1, 2]


@pytest.fixture()
def default_distr() -> UniVarSampleDistr:
    distr = UniVarSampleDistr()
...
def test_observe(default_distr: UniVarSampleDistr):
...
def test_get_support(default_distr: UniVarSampleDistr):
    assert default_distr.get_support() == [1, 2, 3]


def test_get_probabilities(default_distr: UniVarSampleDistr):
    assert default_distr.get_probabilities() == [1 / 2, 1 / 4, 1 / 4]


def test__call__(default_distr: UniVarSampleDistr):
...
def test__lq__(default_distr: UniVarSampleDistr):
...
def test__le__(default_distr: UniVarSampleDistr):
...
def test__ge__(default_distr: UniVarSampleDistr):
...
def test__gt__(default_distr: UniVarSampleDistr):
